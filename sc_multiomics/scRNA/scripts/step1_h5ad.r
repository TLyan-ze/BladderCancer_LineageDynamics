...
scrna_data5 <- Read10X_h5(
"/WorkDir3/DewangZhou/scRNA-M/RC-3/outs/filtered_feature_bc_matrix.h5")
seob5 <- CreateSeuratObject(counts = scrna_data5, 
                            min.cells = 3, 
                            min.features = 200)

type2 ="RT"
seob5@meta.data$type = type2
sample5 = "CTR_RT"
seob5@meta.data$sample = sample5
seob5 <- RenameCells(seob5, new.names = paste0("CTR_RT.", colnames(x = seob5)))

scrna_data6 <- Read10X_h5(
"/WorkDir3/DewangZhou//scRNA-M/RTG/outs/filtered_feature_bc_matrix.h5")
seob6 <- CreateSeuratObject(counts = scrna_data6, 
                            min.cells = 3, 
                            min.features = 200)

type2 ="RT"
seob6@meta.data$type = type2
sample6 = "GEM_RT"
seob6@meta.data$sample = sample6
seob6 <- RenameCells(seob6, new.names = paste0("GEM_RT.", colnames(x = seob6)))
...
library(Seurat)      
#seurat2h5seurat中间过渡 
SaveH5Seurat(seob,filename="all.h5seurat", overwrite = TRUE)
#数据转为最终h5ad格式
Convert("all.h5seurat", dest = "h5ad", overwrite = TRUE) 
