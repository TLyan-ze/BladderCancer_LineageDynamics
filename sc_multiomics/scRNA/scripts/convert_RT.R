...
library(anndata)
library(Seurat)
a <- read_h5ad("scRNA4_2000_RT.h5ad")
b <- a$X
...
ct <- t(ct)
#genes.tsv
write.table(data.frame(rownames(ct),rownames(ct)),file = 'plot_RT/genes.tsv',
            quote = F,sep = '\t',
            col.names = F,row.names = F)
#barcodes.tsv
write.table(colnames(ct),file = 'plot_RT/barcodes.tsv',quote = F,
            col.names = F,row.names = F)


#matrix.mtx 文件是3列，第一列是行号，第二列是列号，第三列是基因表达量
#首先写一个头信息
file="plot_RT/matrix.mtx"
sink(file)
...
tmp=tmp[tmp$exp>0,]
head(tmp)
write.table(tmp,file = 'plot_RT/matrix.mtx',quote = F,
