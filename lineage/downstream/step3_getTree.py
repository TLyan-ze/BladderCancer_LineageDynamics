...
#
file_path = "AP.alleleTable.unfiltered.txt2"
dtype={'intBC':str}
...
allele_table = pd.read_csv(file_path,sep='\t',usecols = ['Tumor','sampleID','cellBC', 'intBC','allele', 'r1', 'r2', 'r3','lineageGrp', 'UMI', 'readCount', 'Cell_Clone'],dtype=dtype)
...
#############################
sampleIDs = allele_table['sampleID'].drop_duplicates()
print(sampleIDs)
CLONES = allele_table['Tumor'].drop_duplicates()


for CLONE in CLONES:
    print(CLONE)
    clone_allele_table = allele_table[allele_table['Tumor'] == CLONE]
    clone_allele_table.to_csv(script_dir +"/data/" + str(CLONE)+".allele.csv",sep='\t',index=True,header=True)
    n_cells = clone_allele_table['cellBC'].nunique()
    a_barcode=clone_allele_table['cellBC'].drop_duplicates()
    a_barcode.to_csv(script_dir +"/data/" + str(CLONE)+".barcode.csv",sep='\t',index=True,header=True)
    ...
    character_matrix.to_csv(script_dir +"/data/" + str(CLONE)+"_character.csv",sep='\t',index=True,header=True)
    priors[0]
    ...
    cas_tree.character_matrix.to_csv(script_dir +"/trees/" + str(CLONE)+"_character_matrix.txt",sep='\t',index=True,header=True)
    ###########################################################图一
    ...
    plt.show()
    plt.savefig(script_dir +"/data/" + str(CLONE)+"character_unique.png")
    plt.savefig(script_dir +"/data/" + str(CLONE)+"character_unique.pdf")
    ...
    plt.show()
    plt.savefig(script_dir +"/data/" + str(CLONE)+"character_missing.png")
    plt.savefig(script_dir +"/data/" + str(CLONE)+"character_missing.pdf")
    print(0)
    ##########################################################################################################
    ...
    out_fp = script_dir +"/trees/" + str(CLONE)+"_tree.nwk"
    with open(out_fp, 'w') as f:
        f.write(a2)
    cas.pl.plot_matplotlib(cas_tree, orient='right', add_root=True,allele_table=clone_allele_table)
    plt.savefig(script_dir +"/data/" + str(CLONE)+"_tree.png")
    plt.savefig(script_dir +"/data/" + str(CLONE)+"_tree.pdf")